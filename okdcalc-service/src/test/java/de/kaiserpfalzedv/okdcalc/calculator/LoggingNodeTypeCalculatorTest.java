 ...
 */
public class LoggingNodeTypeCalculatorTest {
    private static final Logger LOG = LoggerFactory.getLogger(LoggingNodeTypeCalculatorTest.class);
    ...
    private LoggingClusterSizeCalculator service;
    ...
    public void shouldReturnASizingWhenSmallNodesAreGiven() throws NoSolutionException {
        ...
        Set<LoggingSizingResult> result = service.scoreNodetypes(sizingRequest);
        LOG.trace("result: {}", result);

        Set<Long> scores = new HashSet<>(result.size());
        for (LoggingSizingResult r : result) {
            scores.add(r.getScore());
        }

        assert scores.contains(12000L);
        assert scores.contains(18000L);
        assert scores.contains(48000L);
    }

    @Test
    public void shouldAcceptACalculatorForConstructor() {
        new LoggingClusterSizeCalculator(new LoggingPodCalculator());
    }

    @BeforeEach
    public void setUpService() {
        service = new LoggingClusterSizeCalculator();
    }
}
